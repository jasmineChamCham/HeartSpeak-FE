        ...
        <section id="how-it-works" className="relative w-full overflow-hidden bg-background h-screen flex items-center px-6 md:px-12 mt-12 text-foreground">
            <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col justify-center gap-12 md:gap-16 py-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-start gap-12 w-full">
                    ...
                    >
                        <h2 className={`${i18n.language === 'en' ? 'text-[2rem] md:text-[2.75rem]' : 'text-[1.75rem] md:text-[2.25rem]'} font-medium leading-[1.1] tracking-tight text-[#18181A] dark:text-gray-100`}>
                            {t('how_it_works.headline_part1')}<span className={`text-[#2b2b2d] dark:text-gray-300 ${i18n.language === 'vi' ? 'font-normal' : 'font-serif italic font-normal'}`}>{t('how_it_works.headline_part2')}</span>{t('how_it_works.headline_part3')}
                        </h2>
                    </motion.div>
                </div>
                ...
                <div className="grid gap-6 md:gap-8 md:grid-cols-3">
                    {steps.map((step, i) => (
                        <motion.div
                            ...
                            className="group relative flex flex-col gap-4 rounded-[1.5rem] bg-white dark:bg-[#1a1a1f] p-6 md:p-8 border border-black/5 dark:border-white/5 transition-all duration-500 hover:shadow-lg hover:-translate-y-1 h-full min-h-[260px]"
                        >
                            <div>
                                <span className={`text-4xl md:text-5xl font-light text-black/80 dark:text-white/80 ${i18n.language === 'vi' ? '' : 'font-serif italic'}`}>
                                    {step.num}
                                </span>
                            </div>

                            <h3 className="font-display text-2xl font-medium text-[#18181A] dark:text-gray-100 tracking-tight">
                                {step.title}
                            </h3>

                            <p className="text-sm md:text-base font-light leading-relaxed text-black/70 dark:text-white/70 mt-auto pt-8">
                                {step.desc}
                            </p>
                        </motion.div>
                    ))}
